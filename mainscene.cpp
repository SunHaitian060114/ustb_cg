...
void MainScene::playGame()
{
    ...
    player.setSource(QUrl(SOUND_BACKGROUND));
...
void MainScene::updatePosition()
{
    ...
    if(!m_hero.m_Free) {
        // 边界检查
        int newX = m_hero.m_X + m_hero.m_SpeedX;
        int newY = m_hero.m_Y + m_hero.m_SpeedY;

        newX = qBound(0, newX, GAME_WIDTH - m_hero.m_Rect.width());
        newY = qBound(0, newY, GAME_HEIGHT - m_hero.m_Rect.height());
...
void MainScene::paintEvent(QPaintEvent *)
{
    ...
    if (Start_Event == 1 || Start_Event == 3) {
        ...
        painter.drawText(GAME_WIDTH - 200, 50, QString("Score: %1").arg(score));
        ...
        painter.drawText(30, 50, QString("Best: %1").arg(max_score));
    }
        ...
        // 显示历史最高分
        painter.drawText(30, 0 + 50, QString("Best: %1").arg(max_score));
    }
}
...
void MainScene::keyPressEvent(QKeyEvent *event)
{
    if (Start_Event == 1) {
        switch (event->key()) {
        case Qt::Key_W:
            m_hero.m_SpeedY = -HERO_SPEED;  // 向上
            break;
        case Qt::Key_S:
            m_hero.m_SpeedY = HERO_SPEED;   // 向下
            break;
        case Qt::Key_A:
            m_hero.m_SpeedX = -HERO_SPEED;  // 向左
            break;
        case Qt::Key_D:
            m_hero.m_SpeedX = HERO_SPEED;   // 向右
            break;
        default:
